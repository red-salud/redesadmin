app.controller('ContactoProveedorCtrl', ['$scope', '$filter', '$uibModal', '$bootbox', '$log', '$timeout', 'pinesNotifications', 'uiGridConstants', 'blockUI', 
  'ContactoProveedorFactory',
  ...
  ProveedorServices
    ...
    $scope.gridOptions = {
      ...
      multiSelect: false,
      columnDefs: [ 
        { field: 'id', name: 'idcontactoproveedor', displayName: 'ID', width: '75',  sort: { direction: uiGridConstants.DESC} },
    ...
    $scope.metodos.getPaginationServerSide = function(loader) {
      ...
      ContactoProveedorServices.sListar(arrParams).then(function (rpta) { 
    ...
    $scope.btnNuevo = function() { 
      ...
      ContactoProveedorFactory.regContactoModal(arrParams); 
    }
    $scope.btnEditar = function() { 
      ...
      ContactoProveedorFactory.editContactoModal(arrParams); 
    }
...
app.service("ContactoProveedorServices",function($http, $q, handleBehavior) {
    return({
        sListar: sListar,
        sListarContactosDeEsteProveedor: sListarContactosDeEsteProveedor, 
    ...
    function sListar(datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"ContactoProveedor/listar_contacto",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    }  
    function sListarContactosDeEsteProveedor(datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"ContactoProveedor/listar_contactos_este_proveedor",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    }
    function sRegistrar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"ContactoProveedor/registrar",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    }  
    function sEditar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"ContactoProveedor/editar",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    }   
    function sAnular (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"ContactoProveedor/anular",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    } 
...
app.factory("ContactoProveedorFactory", function($uibModal, pinesNotifications, blockUI, uiGridConstants, ContactoProveedorServices,ProveedorServices, ClienteServices) { 
  var interfaz = {
    regContactoModal: function (arrParams) {
      blockUI.start('Abriendo formulario...');
      $uibModal.open({ 
        templateUrl: angular.patchURLCI+'ContactoProveedor/ver_popup_formulario',
        size: 'md',
        backdrop: 'static',
        keyboard:false,
        controller: function ($scope, $uibModalInstance, arrParams) { 
          ...
          $scope.aceptar = function () { 
            ...
            ContactoProveedorServices.sRegistrar($scope.fData).then(function (rpta) {
              if(rpta.flag == 1){
                var pTitle = 'OK!';
                var pType = 'success';
                $uibModalInstance.dismiss('cancel');
      ...
      $uibModal.open({ 
        ...
        controller: function ($scope, $uibModalInstance, arrParams) { 
          ...
          $scope.aceptar = function () { 
            blockUI.start('Procesando información...');
            $scope.fData.origen = 'contactos';
            ContactoProveedorServices.sEditar($scope.fData).then(function (rpta) {
              if(rpta.flag == 1){
                var pTitle = 'OK!';
                var pType = 'success';
                $uibModalInstance.dismiss('cancel');
