	...
	ColaboradorServices
  	...
  	$scope.gridOptions = {
	    ...
	    columnDefs: [ 
	      ...
	      { field: 'nombre_comercial', name: 'nombre_comercial_cli', displayName: 'Nombre Comercial', minWidth: 140 },
	      { field: 'razon_social', name: 'razon_social_cli', displayName: 'Razón Social', minWidth: 160 },
	      { field: 'numero_documento', name: 'numero_documento_cli', displayName: 'RUC', minWidth: 100 },
	      ...
	      { field: 'telefono', name: 'telefono_cli', displayName: 'Teléfono', minWidth: 90 },
	    ...
	    onRegisterApi: function(gridApi) { 
	      ...
	      $scope.gridApi.core.on.filterChanged( $scope, function(grid, searchColumns) {
	        ...
	        paginationOptions.searchColumn = {
	          ...
	          'ce.telefono_cli' : grid.columns[7].filters[0].term,
	      ...
	      controller: function ($scope, $uibModalInstance) { 
	      	...
	      	var paginationOptionsContactos = {
			      ...
			      search: null
				    ...
				    multiSelect: false,
				    columnDefs: [ 
				      { field: 'id', name: 'idcontactocliente', displayName: 'ID', visible: false, width: '50',  sort: { direction: uiGridConstants.DESC} },
				      { field: 'contacto', name: 'nombres', displayName: 'Contacto', width: 140 },
				    ...
				    onRegisterApi: function(gridApiContacto) { 
				      ...
				      $scope.gridApiContacto.core.on.filterChanged( $scope, function(grid, searchColumns) {
				        ...
				        paginationOptionsContactos.searchColumn = {
				          'co.idcontactocliente' : grid.columns[1].filters[0].term,
				          'co.nombres' : grid.columns[2].filters[0].term,
				          'co.telefono_fijo' : grid.columns[3].filters[0].term,
				          'co.anexo' : grid.columns[4].filters[0].term,
					...
					$scope.quitarContacto = function() {
				      ...
				      $bootbox.confirm(pMensaje, function(result) {
				        if(result){
				        	var arrParams = {
				        		idcontactocliente: $scope.fContacto.id 
				        	}
				        	blockUI.start('Procesando información...');
					...
					$scope.actualizarContacto = function() { 
						...
						blockUI.start('Procesando información...'); // ContactoClienteServices.sActualizarContacto
	          ContactoClienteServices.sEditar($scope.fContacto).then(function (rpta) {
	            if(rpta.flag == 1){
	              var pTitle = 'OK!';
	              var pType = 'success';
	              $scope.fContacto = {};
	            ...
	            pinesNotifications.notify({ title: pTitle, text: rpta.message, type: pType, delay: 2500 });
					...
					$scope.metodos.getPaginationServerSideContactos = function(loader) {
					  ...
					  ContactoClienteServices.sListarContactosDeEstaEmpresa(arrParams).then(function (rpta) { 
	          ...
	          ContactoClienteServices.sRegistrar($scope.fContacto).then(function (rpta) {
	            if(rpta.flag == 1){
	              var pTitle = 'OK!';
	              var pType = 'success';
	              $scope.fContacto = {};
