app.controller('ElementoCtrl', ['$scope', '$filter', '$uibModal', '$bootbox', '$log', '$timeout', 'pinesNotifications', 'uiGridConstants', 'blockUI', 
  'ElementoFactory',
  'ElementoServices',
  ...
  CategoriaElementoServices
    ...
    $scope.metodos.getPaginationServerSide = function(loader) {
      ...
      ElementoServices.sListar(arrParams).then(function (rpta) { 
    ...
    $scope.btnEditar = function() { 
      ...
      ElementoFactory.editElementoModal(arrParams); 
    }
    $scope.btnAnular = function() { 
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          ...
          blockUI.start('Procesando información...');
          ElementoServices.sAnular(arrParams).then(function (rpta) {
            if(rpta.flag == 1){
              var pTitle = 'OK!';
              var pType = 'success';
              $scope.metodos.getPaginationServerSide();
...
app.service("ElementoServices",function($http, $q, handleBehavior) {
    ...
    function sListar(datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"Elemento/listar_elemento",
            data : datos
      });
    ...
    function sRegistrar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"Elemento/registrar",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    }
    function sEditar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"Elemento/editar",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    }
    function sAnular (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"Elemento/anular",
            data : datos
      });
      return (request.then(handleBehavior.success,handleBehavior.error));
    }
...
app.factory("ElementoFactory", function($uibModal, pinesNotifications, blockUI, ElementoServices) { 
  var interfaz = {
    regElementoModal: function (arrParams) {
      blockUI.start('Abriendo formulario...');
      $uibModal.open({ 
        templateUrl: angular.patchURLCI+'Elemento/ver_popup_formulario',
        size: 'md',
        backdrop: 'static',
        keyboard:false,
        controller: function ($scope, $uibModalInstance, arrParams) { 
          ...
          $scope.titleForm = 'Registro de Elemento';
          ...
          var myCallBackCC = function() { 
            $scope.fArr.listaCategoriasElemento.splice(0,0,{ id : '0', descripcion:'--Seleccione categoría de elemento--'}); 
          ...
          $scope.aceptar = function () { 
            blockUI.start('Procesando información...');
            ElementoServices.sRegistrar($scope.fData).then(function (rpta) {
              if(rpta.flag == 1){
                var pTitle = 'OK!';
                var pType = 'success';
                $uibModalInstance.dismiss('cancel');
    ...
    editElementoModal: function (arrParams) {
      blockUI.start('Abriendo formulario...');
      $uibModal.open({ 
        templateUrl: angular.patchURLCI+'Elemento/ver_popup_formulario',
        size: 'md',
        ...
        controller: function ($scope, $uibModalInstance, arrParams) { 
          ...
          $scope.titleForm = 'Edición de Elemento';
          $scope.cancel = function () {
            $uibModalInstance.dismiss('cancel');
          }
          // BINDEO TIPO DE ELEMENTO 
          ...
          $scope.aceptar = function () { 
            blockUI.start('Procesando información...');
            ElementoServices.sEditar($scope.fData).then(function (rpta) {
              if(rpta.flag == 1){
                var pTitle = 'OK!';
                var pType = 'success';
                $uibModalInstance.dismiss('cancel');
