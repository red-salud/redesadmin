...
class Acceso extends CI_Controller {
    			...
    			if($loggedUser['estado_us'] == 1){
					...
					if( empty($arrPerfilUsuario['idusuario']) ){ 
						...
						if( isset($arrPerfilUsuario['idusuario']) ){ 
							$this->session->set_userdata('sess_reds_'.substr(base_url(),-20,7),$arrPerfilUsuario);
	...
	public function lista_empresa_admin_session()
	{
		...
		foreach ($lista as $row) { 
			array_push($arrListado, 
				array( 
					'id' => @$row['idusuario'].'-'.@$row['idempresaadmin'], 
					'idusuarioempresaadmin' => @$row['idusuario'].'-'.@$row['idempresaadmin'], 
	...
	public function cambiar_empresa_admin_session(){ 
		$allInputs = json_decode(trim($this->input->raw_input_stream),true); 
		$this->sessionRS = @$this->session->userdata('sess_reds_'.substr(base_url(),-20,7));
		$fila = $this->model_acceso->m_cambiar_empresa_session($allInputs['datos']['idempresaadmin'],$allInputs['datos']['idusuario']); 
		foreach ($fila as $key => $val) {
			$_SESSION['sess_reds_'.substr(base_url(),-20,7)][$key] = $val;
		} 
		if($allInputs['datos']){
			$arrData['flag'] = 1;
	...
	public function getSessionCI(){
		$arrData['flag'] = 0;
		$arrData['datos'] = array();
		if( $this->session->has_userdata( 'sess_reds_'.substr(base_url(),-20,7) ) && 
	...
	public function logoutSessionCI(){
		$this->session->unset_userdata('sess_reds_'.substr(base_url(),-20,7));
        //$this->cache->clean();
        $arrData['flag'] = 1;
